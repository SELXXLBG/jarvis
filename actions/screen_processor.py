...
try:
    import easyocr
    # Initialisation silencieuse (GPU si dispo, sinon CPU)
    import logging
    logging.getLogger("easyocr").setLevel(logging.ERROR)
    _ocr_reader = easyocr.Reader(['fr', 'en'], gpu=True, verbose=False)
    _OCR_OK = True
except ImportError:
    _OCR_OK = False
    print("[ScreenProcess] [!] EasyOCR non installe. Fallback sur l'envoi d'images au cloud.")
    ...
    async def _send_loop(self):
        while True:
            ...
            if self._session:
                ...
                try:
                    if _OCR_OK:
                        print("[ScreenProcess] 🔍 Analyse visuelle (OCR) locale en cours...")
                        # Inférence GPU/CPU locale pour lire l'écran
                        text_list = _ocr_reader.readtext(image_bytes, detail=0)
                        ocr_text = " ".join(text_list)
                        
                        final_prompt = (
                            f"{user_text}\n\n"
                            f"[TEXTE EXTRAIT DE L'ECRAN EN LOCAL PAR L'ORDINATEUR] :\n"
                            f"{ocr_text if ocr_text else '<Aucun texte détecté sur l\'écran>'}"
                        )
                        
                        parts = [{"text": final_prompt}]
                        print(f"[ScreenProcess] ✅ Texte extrait envoyé ({len(ocr_text)} caractères) — 0 pixel envoyé !")
                    else:
                        ...
                        print("[ScreenProcess] ✅ Image envoyée (Méthode Cloud)")
